...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function scheduleLoginRedirect(message) {
        showMessage(message);
    ...
    // Connect the back button before checking whether the user is logged in.
    // This keeps the button usable even while a login redirect is pending.
    if (backBtn) {
    ...
    if (!userId || userId === "undefined" || userId === "null") {
        scheduleLoginRedirect("Please log in before making a reservation. Redirecting...");
        return;
    }
    ...
    fetch(`${API_URL}/api/users/${userId}`)
        ...
        .catch(error => {
            ...
            scheduleLoginRedirect("Failed to load user details. Please login again.");
        });
